...
async fn debug_pane_recorder_start() {
    ...
    let result = PaneRecorder::start(pane_id.clone(), output_dir, 80, 24, None).await;
    ...
    match result {
        ...
        Err(e) => {
            ...
            let pipe_check = Command::new("tmux").args(["pipe-pane", "-t", &pane_id]).output();
            println!(
                "   tmux pipe-pane response: {:?}",
                pipe_check.map(|o| String::from_utf8_lossy(&o.stderr).to_string())
            );
